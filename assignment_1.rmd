...
```
Then we are going to check for $$P(T \leq X) \geq 0.9$$ with $$T \sim B(X, 0.8)$$.
```{r}
first_over_ninety_index <- which(pbinom(numbers, size = 24, prob = p) >= 1 - alpha)[1]
cat('The index of the first element that is over ', p, '%, is', first_over_ninety_index, '.')
...
cat('With ', 1 - alpha, '% confidence we can say that the lower bound of the', p * 100, 'th quantile starts at x =', sorted_vals[first_over_ninety_index], '.')
```
...
$$ P(T\leq X) \geq 0.95 $$ with $$ T \sim B(X,0.9). $$

```{r}
for (n in 1:1000) {
  if (length(which(pbinom(1:n, size = n, prob = 0.9) >= 0.95)) == 2) {
    cat('The sample must be greater or equal to', n, '.')
    break
  }
}
```
