...
This could be analyzed with a parametric procedure such as ANOVA, since the variances between
groups are roughly equal. We would still need to check for the normality of the
data after performing a test, since the F-Test assumes normality.
...
```{r}
kruskal.test(c(a,b),c(rep(1, length(a)), rep(2, length(b))))

kruskal.test(c(a,c,d),c(rep(1, length(a)), rep(2, length(c)),rep(3, length(d))))

kruskal.test(c(b,c,d),c(rep(1, length(b)), rep(2, length(c)),rep(3, length(d))))

```

Nematocits A are different from all of the others.
...
print(X_squared)
crit_val <- qchisq(p=0.05, df=length(f)-1)
print(crit_val)
```
