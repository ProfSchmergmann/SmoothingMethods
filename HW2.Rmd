...
\Leftrightarrow W_n  &> z_{0.05} * \left( \sqrt{\frac{n_1n_2(n+1)}{12} - \frac{n_1n_2\sum_{i=1}^{k}(t_i^3 -t_i)}{12n(n+1)}} \right) +\frac{n_1(n+1)}{2}
\end{align*}
...
lower <- qnorm(1-0.05) *
  sqrt(n_1 * n_2 * (n + 1) / 12 - (n_1 * n_2 * t_sum )/ (12 * n * (n + 1))) + (n_1 * (n + 1) / 2) 
...
t.test(subset(stereogram_data, subset = (group =="NV"))$fusion.time , 
       subset(stereogram_data, subset = (group =="VV"))$fusion.time, 
